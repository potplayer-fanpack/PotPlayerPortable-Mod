...
void CustomQualityList(array<dictionary> &QualityList)
{
	...
	for (int i = 0; i < QualityList.size(); i++)
	{
		...
		QualityList[i]["quality"] = "" + resolution + ", " + fps + "F(" + format + ")";
	}
}
...
string GetCodecName(string type)
{
	...
	type.replace(";,", ",");
	type.replace("mp4a", "aac");
	type.replace("av01", "av1");
						...
						if (formats.isArray())
						{
							for(int j = 0, len = formats.size(); j < len; j++)
							{
								...
								if (format.isObject())
								{
									...
									if (width.isInt() && height.isInt()) item.resolution = formatInt(width.asInt()) + "×" + formatInt(height.asInt());
									if (bitrate.isInt())
									{
